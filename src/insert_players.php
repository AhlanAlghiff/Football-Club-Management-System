...
if (isset($_POST['add_player'])) {
  $nama = trim($_POST['playerName']); // Trim leading/trailing spaces
  ...
  // Validate player image (optional, enhance security)
  $player_image = $_FILES['player_image'];
  $validImageExtensions = array('jpg', 'jpeg', 'png', 'gif'); // Allowed image types
  $hasImageError = false;
  $imageErrorMessage = '';

  // Check for uploaded file and valid extension
  if (!empty($player_image['name'])) {
    $imageExtension = strtolower(pathinfo($player_image['name'], PATHINFO_EXTENSION));
    if (!in_array($imageExtension, $validImageExtensions)) {
      $hasImageError = true;
      $imageErrorMessage = 'Invalid image format. Please upload a JPG, JPEG, PNG, or GIF image.';
    }
  }

  // Check for empty fields and potential image error
  if (empty($nama) || empty($tanggal) || empty($negara) || empty($tinggi) || empty($noPunggung) || empty($posisi) || $hasImageError) {
    $message = 'Please fill out all required fields.';
    if ($hasImageError) {
      $message .= ' ' . $imageErrorMessage; // Append image error message if applicable
    }
  } else {
    // Generate a unique filename (optional, prevents duplicate filename conflicts)
    $player_image_name = uniqid('', true) . '.' . $imageExtension; // Add extension
    $player_image_folder = '../asset/uploaded_img/' . $player_image_name;

    // Prepare the query with prepared statements (security)
    $stmt = mysqli_prepare($conn, "INSERT INTO pemain(nama, tanggal_lahir, kewarganegaraan, height, posisi, nomor_punggung, image) VALUES(?, ?, ?, ?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, 'sssssss', $nama, $tanggal, $negara, $tinggi, $posisi, $noPunggung, $player_image_name);

    $upload = mysqli_stmt_execute($stmt);

    if ($upload) {
      // Move uploaded file with error handling
      if (move_uploaded_file($player_image['tmp_name'], $player_image_folder)) {
        $message = 'Player added successfully, Welcome to our New Player!';
      } else {
        $message = 'Error uploading image. Please try again.';
      }
    } else {
      $message = 'Could not add the player. Error: ' . mysqli_stmt_error($stmt);
    }
        ...
        <div class="relative p-10 flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-xl bg-clip-border">
            <div class="relative mx-auto mt-4 overflow-hidden text-gray-700 bg-white rounded-none bg-clip-border w-full max-w-screen-lg">
                ...
                <div class="relative flex flex-col text-gray-700 bg-transparent shadow-none rounded-xl bg-clip-border">
                    <form role="form" action="insert_players.php" method="post" class="max-w-screen-lg mt-8 mb-2 w-full flex flex-col space-y-4 mx-auto">
                        <!-- Row 1 -->
                        ...
                        <div class="flex flex-col md:flex-row gap-4">
                            ...
                            <div class="relative h-11 w-full min-w-[200px] mb-2">
                                ...
                                <select id="position" name="position" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <div class="relative h-11 w-full min-w-[200px]">
                            <label for="player_image" class="block text-sm font-medium text-gray-700 mb-2">Player Photo</label>
                            <input type="file" id="player_image" name="player_image" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4  mb-4 rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                        </div>
                        <div class="relative h-11 w-full min-w-[200px]">
