...
if(isset($_POST['submit'])){
   ...
   if(empty($presence_name) || empty($match_session) || empty($attend_info) || empty($presence_desc)){
      ...
      $update = "UPDATE `kehadiran` SET `id_sesi` = '$match_session', `id_pemain` = '$presence_name', `status_kehadiran` = '$attend_info', `catatan` = '$presence_desc' WHERE `kehadiran`.`id_kehadiran` = $id;";
      $upload = mysqli_query($conn,$update);
      if($upload){
         $message = 'Update Player Presence added successfully!';
      }else{
         $message = 'could not Update the Presence';
      }
   }
...
<head>
    ...
    <title>FC Management System | Add Presence Player</title>
</head>
<body>
